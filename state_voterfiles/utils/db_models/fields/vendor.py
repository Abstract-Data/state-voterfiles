...
class VendorTagsToVendorLink(SQLModelBase, table=True, link_model=True):
    ...
    vendor_id: int | None = SQLModelField(default=None, foreign_key=f"vendor_tags.id", primary_key=True, unique=True)
    tag_id: int | None = SQLModelField(default=None, foreign_key=f"vendor_name.id", primary_key=True, unique=True)


class VendorTags(SQLModelBase, table=True):
    __tablename__ = 'vendor_tags'
    id: int | None = SQLModelField(default=None, primary_key=True)
    tags: Dict[str, Any] = SQLModelField(..., description="List of tags associated with the vendor", sa_type=JSON)
    vendors: list["VendorName"] = Relationship(back_populates="tags", link_model=VendorTagsToVendorLink)
...
class VendorTagsToVendorToRecordLink(SQLModelBase, table=True, link_model=True):
    ...
    vendor_tag_link_id: int | None = SQLModelField(default=None, foreign_key=f"{VendorTagsToVendorLink.__tablename__}.vendor_id", primary_key=True)
    record_id: int | None = SQLModelField(default=None, foreign_key="recordbasemodel.id", primary_key=True)
